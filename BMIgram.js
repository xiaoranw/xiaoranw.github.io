...
function renderMyBMIgram() {
	...
	d3.csv("finalprojectTable_FinalA.csv", function (data) {
		...
		d3.json("us-states.json", function (json) {
			...
			var select = d3.select("body").append("select").on("change", svg),
				options = select.selectAll('option').data(json) // Data join
					.attr("width", 800)
					.attr("height", 200)
			// Enter selection
			options.enter("1990").append("option").text(function (d,i) { return d.properties.obesrate2016; });
